...
Die Bedeutung dieser Frage ergibt sich aus dem zunehmenden Wettbewerb unter den Gastgebern, die bestrebt sind, ihre Unterkünfte attraktiv zu gestalten und gleichzeitig optimale Preise zu erzielen. Durch die Analyse von Daten aus den Regionen Zürich, Genénve und Vaud versucht diese Arbeit, die Schlüsselelemente zu identifizieren, die die Attraktivität und die Preisgestaltung beeinflussen. Neben der geografischen Lage, der Art der Unterkunft, der Anzahl der Zimmer und Bäder sowie der Verfügbarkeit von Annehmlichkeiten werden auch Faktoren wie die Erfahrung und das Profil des Gastgebers analysiert. Diese Untersuchung zielt darauf ab, die prädiktiven Merkmale einer Unterkunft zu bestimmen, die als Indikatoren für höhere Preise fungieren, und präskriptive Empfehlungen für Gastgeber zu entwickeln, um die Attraktivität ihrer Unterkünfte zu steigern.
...
ETL \[3pt\] Jovan How qualitative (detailed) is the ETL/EDA process done? \[1pt\] What are the findings and how are they discussed? \[1pt\] Was the data set cleaned with reasonable approaches? \[1pt\]
...
df_zuerich_cleanead <- df_zuerich %>%
  mutate(price_in_usd = as.numeric(ifelse(grepl("\\$[0-9.]+", price), gsub("\\$", "", price), NA))
         ) %>%
...
```{r}
# Exportieren des DataFrames df_zuerich_cleanead in eine CSV-Datei
...
*Data analysis \[2pt\] Cedric / Jovan Are data attributes clear and obvious discussed? \[1pt\] Are adequate EDA methodologies used for data analysis? \[1pt\]*
...
ggplot(df_zuerich_cleanead, aes(x = calculated_host_listings_count_private_rooms, y = price_in_usd)) +
  geom_point(aes(color = host_is_superhost), alpha = 0.5) +
  labs(x = "Anzahl Listings pro Hos", y = "Preis (USD)", color = "Host ist Superhost", title = "Negative Beziehung zwischen Anzahl Listings pro Host und Preis") +
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  theme_minimal()
