...
In der heutigen dynamischen Welt des Online-Tourismus ist Airbnb eine zentrale Plattform für die Buchung von Unterkünften. Diese Arbeit untersucht die Faktoren, die den Preis pro Person einer Airbnb-Unterkunft beeinflussen, um Gastgebern datenbasierte Empfehlungen zur Optimierung ihrer Preisstrategien zu geben. Mithilfe von deskriptiver Statistik, Korrelationsanalysen und linearen Regressionsmodellen analysieren wir die Daten von Airbnb-Unterkünften in Zürich, Vaud und Genf. Die Ergebnisse zeigen, dass die Sauberkeitsbewertung der Unterkünfte, die Anzahl der Listings eines Hosts sowie die Verfügbarkeit wesentliche Einflussfaktoren sind. Basierend auf diesen Erkenntnissen entwickeln wir prädiktive Modelle zur Vorhersage des Preises pro Person. Unsere Analyse bietet Einblicke für Gastgeber, die ihre Preisstrategien verbessern und ihre Einnahmen maximieren möchten.

# Einleitung (mit Forschungsfrage (d.h. Geschäftsfrage) am Ende)
...
**Präskriptive Analyse:** In diesem Dokument wird keine präskriptive Analyse umgesetzt.

# Datenquelle (mit Angaben zu Quelle, Qualität und Bereinigungsschritten der Daten)
...
# Datenqualität (Analyse im Hinblick auf Datenqualitätsaspekte)
...
# Datenanalyse (Informationen zur Datenstruktur, Organisation und zu den für die Analyse verwendeten Methoden)
...
Die deskriptive Statistik liefert grundlegende Informationen über die Struktur und Verteilung der Daten. Ziel ist es, einen Überblick über die Daten zu bekommen und mögliche Anomalien oder interessante Muster zu erkennen. Wir möchten damit ein grundlegendes Verständnis über die Datenstruktur und die Verteilung der Variablen erlangen. Dafür untersuchen wir die Attribute Preis pro Person und Bewertung.
...
combined_data <- bind_rows(
  mutate(df_zuerich_cleaned, location = "Zurich"),
  mutate(df_vaud_cleaned, location = "Vaud"),
  mutate(df_geneva_cleaned, location = "Geneva")
)

# Deskriptive Statistik für numerische Variablen
summary_price <- combined_data %>%
  group_by(location) %>%
  summarise(mean_price = mean(price_per_person), median_price = median(price_per_person))

summary_rating <- combined_data %>%
  group_by(location) %>%
  summarise(mean_rating = mean(review_scores_rating), median_rating = median(review_scores_rating))

print(summary_price)
print(summary_rating)

# Histogramm für die Verteilung des Preises pro Person
histogram_price <- ggplot(combined_data, aes(x = price_per_person, fill = location)) +
  geom_histogram(binwidth = 10, color = "black", alpha = 0.7, position = "dodge") + # color = NA, um den schwarzen Rand zu entfernen
  labs(title = "Verteilung des Preises pro Person", x = "Preis pro Person", y = "Häufigkeit") +
  theme_minimal() +
  facet_wrap(~location) +
  xlim(0, 300)  # Begrenzung der x-Achse auf maximal 300

# Histogramm für die Verteilung der Bewertung
histogram_rating <- ggplot(combined_data, aes(x = review_scores_rating, fill = location)) +
  geom_histogram(binwidth = 1, color = "black", alpha = 0.7, position = "dodge") +
  labs(title = "Verteilung der Bewertung", x = "Bewertung", y = "Häufigkeit") +
  theme_minimal() +
  facet_wrap(~location)

# Boxplot für den Preis pro Person
boxplot_price <- ggplot(combined_data, aes(x = location, y = price_per_person, fill = location)) +
  geom_boxplot() +
  labs(title = "Boxplot des Preises pro Person", x = "Ortschaft", y = "Preis pro Person") +
  theme_minimal() +
  ylim(0, 500)

# Boxplot für die Bewertung
boxplot_rating <- ggplot(combined_data, aes(x = location, y = review_scores_rating, fill = location)) +
  geom_boxplot() +
  labs(title = "Boxplot der Bewertung", x = "Ortschaft", y = "Bewertung") +
  theme_minimal() +
  ylim(2, 5)

# Anzeigen der Grafiken
print(histogram_price)
print(histogram_rating)
print(boxplot_price)
...
Die Korrelationsanalyse untersucht die Stärke und Richtung der Beziehung zwischen verschiedenen numerischen Variablen. Pearson-Korrelationskoeffizienten wurden berechnet, um zu quantifizieren, wie stark zwei Variablen miteinander variieren. Die Idee ist, Zusammenhänge zwischen Variablen zu finden, insbesondere zwischen den Bewertungen, Host-Informationen, geografischen Daten und dem Preis pro Person. Dafür berechnen wir den Korrelationskoeffizienten und gehen auf die Top 5 der positiven und negativen Korrelationen ein.
...
Die lineare Regression wurde verwendet, um die Beziehung zwischen einer abhängigen Variable (Preis pro Person) und einer oder mehreren unabhängigen Variablen zu modellieren. Dies hilft, die Auswirkungen der unabhängigen Variablen auf die abhängige Variable zu quantifizieren und Vorhersagen zu treffen. Dafür untersuchen wir den Einfluss einzelner Variablen (z.B. Bewertungen, Host-Attribute, Entfernung zum Stadtzentrum) auf den Preis pro Person, um unsere Hypothesen zu überprüfen. Weiterhin erstellen wir mehrere lineare Regressionsmodelle, um den Einfluss spezifischer unabhängiger Variablen auf die abhängige Variable (Preis pro Person) zu analysieren.
...
Verschiedene Arten von Plots wurden verwendet, um die Ergebnisse der deskriptiven Statistik, der Korrelationsanalyse und der Regressionsmodelle zu visualisieren. Das Ziel der Visualisierung ist es, eine verständliche Darstellung der Ergebnisse zu bieten und Muster in den Daten zu identifizieren.
...
# Ergebnisse anzeigen 
print("Vorhersagemodell für Zürich")
cat("RMSE:", rmse, "\n") 
cat("MAE:", mae, "\n") 
cat("R²:", r2, "\n")  
...
-   **R²: 0.7960764** – Ein R²-Wert von 0.80 deutet darauf hin, dass das Modell die Variation des Preises pro Person gut erklärt und einen grossen Teil der Variabilität im Preis vorhersagen kann.
...
# Ergebnisse anzeigen 
print("Vorhersagemodell für Vaud")
cat("RMSE:", rmse, "\n") 
cat("MAE:", mae, "\n") 
cat("R²:", r2, "\n")  
...
-   **R²: 0.7177398** – Ein R²-Wert von 0.72 deutet darauf hin, dass das Modell die Variation des Preises pro Person gut erklärt und einen grossen Teil der Variabilität im Preis vorhersagen kann.
...
# Ergebnisse anzeigen 
print("Vorhersagemodell für Genf")
cat("RMSE:", rmse, "\n") 
cat("MAE:", mae, "\n") 
cat("R²:", r2, "\n")  
...
# Ergebnisse (statistische Ergebnisse, Zahlen, Diagramme)
...
Die deskriptive Analyse der Airbnb-Daten aus Zürich, Vaud und Genf zeigt signifikante Unterschiede in der Verteilung der Preise pro Person und der Bewertungsscores in den drei Städten.

-   **Preis pro Person**: In allen drei Städten zeigt sich eine breite Preisverteilung, wobei die meisten Preise pro Person im Bereich von 50 bis 150 USD liegen. Vaud weist tendenziell geringfügig niedrigere Preise pro Person auf als Zürich und Genf.

-   **Bewertungen**: Die Bewertungen der Unterkünfte sind insgesamt sehr hoch, meist im Bereich von 80 bis 100 Punkten. Zürich und Genf zeigen ähnliche Bewertungsverteilungen, während Vaud etwas variablere Bewertungen aufweist.

-   **Ausreisser und Anomalien**: Es gibt einige Ausreisser in den Preisdaten, insbesondere in den höheren Preisbereichen. Diese könnten auf besonders luxuriöse oder grossräumige Unterkünfte hinweisen. Die Bewertungsscores zeigen weniger Ausreisser, was auf eine allgemein hohe Zufriedenheit der Gäste hinweist.

```{r}
cat("Preis pro Person in Zurich \n")
...
```{r}
cat("Preis pro Person in Geneva \n")
...
Mithilfe der linearen Regression sollen die Hypothesen beantwortet werden, die wir zu Beginn aufgestellt haben.
...
plot_correlations <- function(cor_df, title, num_top_bottom = 10) {
  ...
  ggplot(cor_df, aes(x = reorder(Variable, Correlation), y = Correlation)) +
    ...
    coord_flip() +
    theme_minimal() +
    labs(title = title, subtitle = "Die 20 Variablen mit dem höchsten Wert", x = "", y = "Korrelation")
}


# Erstellung der Plots
plot_zurich <- plot_correlations(zurich_cor, "Korrelationen mit preis pp. in Zürich")
plot_vaud <- plot_correlations(vaud_cor, "Korrelationen mit preis pp. in Vaud")
plot_geneva <- plot_correlations(geneva_cor, "Korrelationen mit preis pp. in Genf")
...
### Lineare Regression für Zürich, Vaud und Geneva
...
# Funktion zur Erstellung eines Streudiagramms mit Regressionslinie für mehrere Variablen mit einem Titel
plot_multiple_regressions_single_title <- function(data, x_vars, y_var, main_title) {
  ...
  grid.arrange(grobs = plots, ncol = 1, top = main_title)
}

# Variablen für Bewertungen, Gastgeber und Lage
rating_vars <- c("review_scores_rating", "review_scores_cleanliness", "review_scores_communication")
host_vars <- c("host_is_superhost", "host_identity_verified", "host_listings_count")
location_vars <- c("longitude", "latitude")

# Plot für Bewertungen in Zürich
plot_multiple_regressions_single_title(df_zuerich_cleaned, rating_vars, "price_per_person", "Zürich - Bewertungen")

# Plot für Gastgeber in Zürich
plot_multiple_regressions_single_title(df_zuerich_cleaned, host_vars, "price_per_person", "Zürich - Gastgeber")

# Plot für Lage in Zürich
plot_multiple_regressions_single_title(df_zuerich_cleaned, location_vars, "price_per_person", "Zürich - Lage")

# Plot für Bewertungen in Vaud
plot_multiple_regressions_single_title(df_vaud_cleaned, rating_vars, "price_per_person", "Vaud - Bewertungen")

# Plot für Gastgeber in Vaud
plot_multiple_regressions_single_title(df_vaud_cleaned, host_vars, "price_per_person", "Vaud - Gastgeber")

# Plot für Lage in Vaud
plot_multiple_regressions_single_title(df_vaud_cleaned, location_vars, "price_per_person", "Vaud - Lage")

# Plot für Bewertungen in Geneva
plot_multiple_regressions_single_title(df_geneva_cleaned, rating_vars, "price_per_person", "Geneva - Bewertungen")

# Plot für Gastgeber in Geneva
plot_multiple_regressions_single_title(df_geneva_cleaned, host_vars, "price_per_person", "Geneva - Gastgeber")

# Plot für Lage in Geneva
plot_multiple_regressions_single_title(df_geneva_cleaned, location_vars, "price_per_person", "Geneva - Lage")

```

### Schlussfolgerung

Die verschiedenen Grafiken und deren lineare Regressionen liefern interessante Einblicke in die Beziehung zwischen den untersuchten Variablen und dem Preis pro Person. Allerdings zeigen die Ergebnisse, dass viele der unabhängigen Variablen nur einen geringen Einfluss auf den Preis pro Person haben. Dies könnte darauf hindeuten, dass andere, nicht berücksichtigte Faktoren eine grössere Rolle spielen oder dass die Märkte in diesen Städten relativ homogen sind. Weitere Untersuchungen könnten sich auf zusätzliche Variablen oder nicht-lineare Modelle konzentrieren, um potenzielle Einflussfaktoren besser zu identifizieren.
...
# Schlussfolgerung (Beantwortung der Frage)
...
Die Analyse zeigt, dass wir den Preis pro Person einer Unterkunft erfolgreich prognostizieren können, wobei die besten Prädiktoren der Preis in USD, die Anzahl der privaten Zimmer und die Kapazität der Unterkunft sind. Für Zürich und Vaud liefern die Modelle relativ genaue Vorhersagen (R² von 0.796 und 0.718), während das Modell für Genf eine geringere Genauigkeit aufweist (R² von 0.532) und Verbesserungspotenzial hat.
...
Die Inhalte dieses Papers wurden eigenständig erarbeitet. Bei der Formulierung und Strukturierung bestimmter Textpassagen wurde ChatGPT[@openai-2022] zur Hilfe genommen, um Korrekturen vorzunehmen und die Kohärenz zu verbessern.
